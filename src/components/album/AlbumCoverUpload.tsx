...
export function AlbumCoverUpload({ albumId, imageUrl }: AlbumCoverUploadProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      {/* Zone image — cliquable seulement pour l'admin */}
      {isAdmin ? (
        <label
          htmlFor={inputId}
        ...
        <div className="relative w-36 h-36 sm:w-44 sm:h-44 shrink-0 shadow-2xl rounded-sm overflow-hidden bg-[#282828]">
          {currentUrl ? (
            <Image src={currentUrl} alt="Pochette" fill sizes="(max-width:640px) 144px, 176px" className="object-cover" />
          ) : (
            <div className="w-full h-full flex items-end p-4 bg-gradient-to-br from-[#444] to-[#1a1a1a]">
      ...
      {isAdmin && (
        <>
          ...
          >
            {uploading ? (
              <><span className="w-3 h-3 rounded-full border-2 border-white/20 border-t-white/60 animate-spin" />Upload en cours…</>
            ) : (
              <><Upload size={11} />Changer la pochette</>
            )}
          </label>
          <input
            id={inputId}
